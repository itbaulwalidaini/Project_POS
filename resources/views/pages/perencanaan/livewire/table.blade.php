<div>
    ...
    <div class="default-table-area mx-minus-1">
            ...
            <table class="table align-middle">
                <thead>
                    <tr>
                        ...
                        <th class="fw-medium" scope="col">Kode</th>
                        <th class="fw-medium" scope="col">Jumlah Item</th>
                        <th class="fw-medium" scope="col">Catatan</th>
                ...
                <tbody>
                    @forelse ($perencanaan as $baris)
                    <tr>
                        <td class="text-body">{{ str_pad($perencanaan->firstItem() + $loop->index, 2, '0', STR_PAD_LEFT) }}.</td>
                        <td class="text-body fw-bold">{{ $baris->kode }}</td>
                        <td class="text-body">{{ $baris->jumlah_item }}</td>
                        <td class="text-body">{{ $baris->catatan ?? '-' }}</td>
                        ...
                        <td>
                            <div class="menu d-flex justify-content-start" style="gap: 12px;">
                                ...
                                <button class="bg-transparent p-0 border-0 hover-text-danger" data-bs-placement="top" data-bs-title="Hapus Data" data-bs-toggle="tooltip"
                                    onclick="confirmDelete('{{ $baris->id }}')"><i class="material-symbols-outlined fs-25 fw-normal text-body">delete</i>
                                </button>
                                @endif
                            </div>
                        </td>
        ...
        <div class="d-flex justify-content-center justify-content-sm-between align-items-center text-center flex-wrap gap-2 showing-wrap pt-15 p-17">
            {{ $perencanaan->links('vendor.pagination.custom') }}
        </div>
    </div>
</div>